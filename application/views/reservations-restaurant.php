								...
								<div class="fg-line ">
									<select id="server" class="selectpicker" >
												...
												foreach($servers as $row){
													echo "<option value=".$row->user_id." id="."server_id".$row->user_id.">".$row->name."</option>";
												}
												?>
											</select>
								</div>
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function(){
				var table = $("#data-table-command").bootgrid({
					...
					url: "<?php echo base_url();?>"+"Reservations/get_reservations",
				formatters: {
				...
				$('#save-reservation').unbind('click').bind('click', function(){
					...
					$ .ajax ({
						url: "<?php echo base_url(); ?>"+"reservations/update_reservation" ,
								data: {'restaurant_id': user_id,'user_id': reservations[pos].user_id,
			...
			function deleteReservation(rowid){
				...
				swal({   
								...
								$ .ajax ({
									...
									data: {'restaurant_id': user_id, 'user_id': reservations[pos].user_id,
										'reservation_id':rowid,'server_id': reservations[pos].server_id,
										'<?php echo $this->security->get_csrf_token_name(); ?>': '<?php echo $this->security->get_csrf_hash(); ?>'
										},
									type: 'POST' ,
									ContentType: 'application/json',
